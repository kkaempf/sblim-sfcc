 ...
 * Description:
 *
 *  Test for invokeMethod() library API. Tests uint32_t data type handling.
...
/*---------------------------------------------------------------------*/
    ...
    printf("+++T2:returning uint32_t value\n");

    retval = cc->ft->invokeMethod(
        cc, objectpath, "Method_uint32", NULL, NULL, &status);
...
/*---------------------------------------------------------------------*/
    ...
    printf("+++T3:passing OUT uint32_t value\n");

    args = newCMPIArgs(NULL);

    retval = cc->ft->invokeMethod(
        cc, objectpath, "Method_uint32_out", NULL, args, &status);
    ...
    if (args) {
        char     *cv;
        CMPIData data = CMGetArg(args, "Arg_uint32", NULL);
        if (!CMIsNullValue(data))
        {
            cv = value2Chars(data.type,&(data.value));
            printf("\n result(s): OUT Parm:Arg_uint32 type: %d value:%s\n",
                                                            data.type, cv);
            if (cv != NULL) free(cv);
            CMRelease(args);
        }
